...
# Re = 120, flow velocity = 0.48m/s, 110 mu m deployment depth
...
if PROFILE
  ...
  smc = NativeInstance.new('smc', 'map_kernel', args: '--profile mpiexec -np '+ENV["QCG_KERNEL_smc"]+" #{isr2d_bindir}/SMCController")
else
  #smc = NativeInstance.new('smc', "#{isr2d_bindir}/SMCController")
  smc = MPIInstance.new('smc', "#{isr2d_bindir}/SMCController", mpiexec: mpiexec, mpiexec_args: '-np '+ENV["QCG_KERNEL_smc"])
end
# SMC2D output mapper
...
if PROFILE
  #bf = NativeInstance.new('bf', 'map_kernel', args: "--profile --no-mpi #{isr2d_bindir}/FlowController")
  #bf = NativeInstance.new('bf', 'map_kernel', args: '--profile -np '+ENV["QCG_KERNEL_bf"]+" #{isr2d_bindir}/FlowController")
  bf = NativeInstance.new('bf', 'map_kernel', args: '--profile mpiexec -np '+ENV["QCG_KERNEL_bf"]+" #{isr2d_bindir}/FlowController")
else
  #bf = NativeInstance.new('bf', "#{isr2d_bindir}/FlowController")
  bf = MPIInstance.new('bf', "#{isr2d_bindir}/FlowController", mpiexec: mpiexec, mpiexec_args: '-np '+ENV["QCG_KERNEL_bf"])
end
