...
if not CONST_TURB
  ...
  when "GEM"
    if PROFILE
      turb = NativeInstance.new('turb', 'map', args: '--profile ' + MPICMD + ' -np '+ENV["QCG_KERNEL_turb"]+' '+KDIR+'/gem_kernelB')
    else 
      if DEBUG_ALLINEA
        turb = MPIInstance.new('turb', KDIR+'/gem_kernelB', mpiexec: MPICMD, mpiexec_args: '-np '+ENV["QCG_KERNEL_turb"])
      else
        turb = MPIInstance.new('turb', KDIR+'/gem_kernelB', mpiexec: MPICMD, mpiexec_args: '-np '+ENV["QCG_KERNEL_turb"])
      end
    end
  when "DFEFI"
    turb = MPIInstance.new('turb', KDIR+'/dfefi_kernelB', mpiexec: MPICMD, mpiexec_args: '-np '+ENV["QCG_KERNEL_turb"])
  else
